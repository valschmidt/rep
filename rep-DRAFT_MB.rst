...
Finally, installation of a MBES aboard a vehicle is accompanied by a calibration procedure called a "Patch Test". A Patch Test is a set of data whose collection is designed to isolate and measure angular errors in the sonar's coordinate reference frame with respect to the filtered output of an IMU's reference frame. IMUs used in operation of MBESs contain internal filters that report attitude (roll, pitch and heading) with respect to a local reference frame established by the local gravitational field and either magnetic or other measure of True heading (Dual GPS, earth rotation, etc.).  
...
..
   ...
   End:
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEyODk3OTE5MTEsMTI4ODMzMjIwMiwyMT
